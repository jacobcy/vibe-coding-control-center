...
**vibe-task 标签要求**：
- 发布任何 `[governance suggest]` 或 `[governance auto-recover]` 评论后，**必须**立即添加 `vibe-task` 标签
- `vibe-task` 标签表示该 issue 已被 governance 审查过，作为"已审查"标记
- 如果需要重新审查某个 issue，应先移除 `vibe-task` 标签（人类也可以手动移除）
...
- **vibe-task 标签检查**：如果 issue 已有 `vibe-task` 标签，直接跳过（已审查过）
