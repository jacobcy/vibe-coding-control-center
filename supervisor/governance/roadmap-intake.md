...
**硬判定标准**（满足任一得 1 分）：
- issue 提议新增一个非必要的命令/API，但未通过"命令准入三问法"验证 → 违反 Skill-First 原则（HARD RULES #16）
  - **三问法**（详见 CLAUDE.md HARD RULES #16）：
    1. 是否创造不可替代的原子能力？
    2. Skill 编排现有命令能否达到 80% 目标？
    3. 是否属于核心管线阶段？
  - **典型反例**（PR #3041）：
    - 一次性数据修复（如"清理历史污染"）→ 应使用 `temp/` 脚本
    - 边缘场景工具（仅服务 < 5 次/月触发）→ 应使用 Skill 或脚本
    - issue body 有"或手动修复"选项 → 说明脚本足够
...
**定义**：为仅执行一次的修复操作新增正式命令或长期模块

**硬判定标准**（满足任一得 1 分）：
- issue 目标明确为"修复历史数据"、"清理无效记录"、"一次性迁移"等单次操作
- issue 提议新增 Python 命令或长期 Repository 方法，而非 Shell 脚本
- issue body 中有"或手动修复"、"或执行 SQL"等选项 → 说明脚本足够

**正确做法**：
- ✅ 创建 `temp/fix_xxx.sh` 脚本（临时文件，不提交到 git）
- ✅ 执行修复 → 验证 → 保留在 `temp/` 作为记录
- ✅ 需要时从 git history 找回（如果提交过）

**通过条件**（不得分）：
- issue 明确说明需要持续性工具（如"每周巡检"、"CI 集成"、"未来可能重复"）
- 修复逻辑需要集成到已有流程中（如 write guard、dispatcher check）

**案例参考**：PR #3041（已关闭）—— 一次性修复脚本（95 行 Shell）vs 过度设计命令（535 行 Python）
