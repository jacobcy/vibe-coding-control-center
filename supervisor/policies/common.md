...
跨对话长期记忆，保存架构共识、决策、最佳实践。**无 `claude-memory` CLI**——用 `/mem-search` skill（封装 MCP search 工具）。
...
**新增**：plan 目标的技术可行性是否已验证（验证前提假设是否成立）。
**新增**：预估 PR 规模（文件数量、变更行数范围）。
