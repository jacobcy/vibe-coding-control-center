...
- 用 exact change partitions 确认 review 范围，用 Kernel 命中确定最低审查深度
...
- **历史 verdict**（`/mem-search`，3-layer，可选）：相似 issue/变更的历史 review 决策可参考时，`search` → 过滤相关 ID → `get_observations` 查历史 verdict/pitfall。memory 非真源，不覆盖当前 diff/ADR/inspect 证据。
...
3. **Plan 已声明死代码清理，删除的符号在声明列表中**：
   - ✅ 符合规则
   - 验证 executor 是否提供了入口审计、精确搜索和目标测试等独立证据
...
- **是否只看 diff，没有收集补充证据？**
  - 用 `inspect base/files/symbols` 收集 Git、AST、Kernel 和正向引用证据
  - 不把这些证据描述成完整运行时影响面
...
- 影响 inspect 证据解释或 Kernel review policy 的逻辑
...
优先把项目自己的 Git/AST/Kernel 证据与 diff、测试和运行时验证组合使用。

推荐：
- 用 `vibe3 handoff status` 查看当前 flow 现场
- 用 `vibe3 inspect base --json` 看精确改动和 Kernel review depth
- 需要澄清声明范围或直接 imports 时，用 `vibe3 inspect files <file.py> --json`
- 删除、重命名、签名变化或 Kernel 符号需要正向引用证据时，用 `vibe3 inspect symbols <file.py>:<symbol> --json`

不要机械地对每个文件或符号运行 inspect。inspect 不输出完整影响或风险结论；reviewer 必须结合 diff、测试和运行时证据形成判断。
