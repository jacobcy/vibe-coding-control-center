...
- 对关键符号显式运行 `inspect symbols <file.py>:<symbol>`，并用测试或运行时证据补足静态观察
...
- 用 `vibe3 inspect files <file.py>` 看单文件 AST 事实
- 用 `vibe3 inspect symbols <file.py>:<symbol>` 看已验证的正向引用

inspect 不输出完整影响或风险结论；reviewer 必须结合 diff、测试和运行时证据形成判断。
