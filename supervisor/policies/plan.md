...
- **验证方式**：
  - 对于激活/启用功能类目标，执行实际的 import 测试或测试运行
  - 验证命令模板：
    ```bash
    # 直接子模块导入（验证子模块自身无循环依赖）
    uv run python -c "from vibe3.<module>.<submodule> import <Symbol>"
    ...
    # 测试运行验证
    uv run pytest <test_path> -k <test_name>
    ```
  - 失败处理：如果验证发现障碍（如循环依赖、缺失依赖），必须在 plan 中标注为 `REQUIRED:BEFORE_CODING` 或直接记录为 blocker

- **如果发现前提不成立**：
  ```bash
  uv run python src/vibe3/cli.py handoff append "Plan 技术前提验证失败：<具体问题>" --kind finding --actor "<actor>"
  ```
  - 不要继续规划，等待 manager 指示
...
必须包含：
- **Summary**：一句话说明实现策略
- **Changes**：按文件列出改动意图，必须包含完整的文件路径（如 `src/vibe3/domain/queue_dirty.py`），而非仅文件名或变更类型描述
