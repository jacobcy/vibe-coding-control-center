...
- 判断本次任务属于哪一类改动
- **命名一致性/文档同步/API 签名变更任务**：使用 inspect 和 rg 搜索所有层，确认需要同步更新的完整文件列表，再圈定 plan 范围（项目专属的跨层检查规则见 plan.policy@project）
- **检查 ADR 约束**：先读取项目 ADR 索引（通常为 `docs/decisions/INDEX.md`），再读取相关 accepted ADR 正文，确认计划不违反任何当前有效 ADR。若需偏离，必须在 plan 中显式提议 supersede，而非静默违反。
- **验证 plan 目标的技术可行性**：如果 plan 涉及激活/启用功能，必须先用实际命令验证前提条件（import 测试、测试运行、依赖检查）。发现障碍立即标注为 `REQUIRED:BEFORE_CODING` 或记录为 blocker。验证命令模式见 plan.policy@project。

优先工具见公共规则；规划阶段通常至少会用到 handoff status / show、inspect symbols / files / base、以及 Terminal Decision 判定。
...
- manager 仍需独立验证（见 manager.md 中的 Terminal Decision Contract）
- plan agent 只做判断 + 证据输出，不做执行
- close 决策权归属 manager，这是 plan 和 manager 的职责边界

**关键**：即使 terminal 场景，plan_ref 注册也是强制要求。跳过 plan_ref 注册会导致 manager 在 `state/handoff` 无法找到 refs。
...
- **验证包级导入（通过 `__init__.py` 重导出链）**
  - 如果计划涉及模块重组、re-export 修改、符号移动，必须测试包级导入无循环依赖
  - 项目专属验证命令见 plan.policy@project

- **如果发现冲突**：用 handoff append 记录"Plan 前提不成立：<具体冲突点>"，不要继续规划，等待 manager 指示或调整方案
...
- **如果发现可以复用**：用 handoff append 记录"发现可复用模式：<模式位置和用法>"

### 3. 每一步都可执行且可验证吗？

- **检查步骤依赖关系**：每一步是否依赖前一步完成？是否存在循环依赖或跳步？
- **检查验证方式**：每一步是否都指定了验证方式？验证方式是否与改动类型匹配？

### 4. Scope boundary 是否清晰？

- **每个步骤是否标注了变更类型？**（如：`[import-only]`、`[re-export]`、`[behavior-change]`）
- **是否存在不在允许清单中的变更？** 如果发现需要 plan scope 外的变更，blocked 回 manager。

### 5. Plan 目标的技术前提是否已验证？

- **是否假设了前提条件已满足？**（如：假设某功能可直接激活但未验证循环依赖、假设依赖已存在但未检查安装）
- **验证方式**：执行实际的 import 测试、测试运行或依赖检查（项目专属命令模板见 plan.policy@project）。
- **如果前提不成立**：用 handoff append 记录，等待 manager 指示或显式标注为 blocker。
...
2. **验证传播可达性**：检查中间层 except 是否吞没异常、异常类型是否匹配
3. **标注未验证风险**：如果无法确认中间层行为，用 handoff 记录
...
1. **本计划不包含死代码清理**（默认）
   ...
   - 用 handoff append `--kind finding` 记录，留给后续独立 issue

2. **本计划包含死代码清理，范围为：`<具体文件:符号列表>`**
   - 必须列出具体符号名和 inspect symbols 引用计数为零的证据
   - 清理范围仅限于显式列出的符号，不得扩展

**禁止**：顺带清理未声明的死代码、在 plan scope 外纳入清理任务。
...
1. **允许的变更类型**：逐项列出
2. **禁止的变更类型**：逐项列出
3. **变更类型标签**：每个 Implementation Step 标注（如 `[import-only]`、`[re-export]`、`[test-update]`）

### Scope 自检

计划完成后逐一回答：
- 所有变更是否仅限于声明的允许范围？
- 变更是否不涉及删除模块或方法？（涉及删除需 issue scope 明确授权）
- 变更是否不涉及内联或重构？
- 变更是否不涉及行为修改？
- 所有步骤的变更类型标签是否都在允许清单中？

### Scope 扩展流程

如果规划过程中发现需要 scope 外变更才能完成目标：
- **禁止自行扩展 scope**
- 用 handoff append 记录原因
- 进入 `state/blocked`，请求 manager 确认

### PR 规模预估

Planner 应在 plan 中预估变更规模：

1. **预估变更文件数量**：区分源码、测试、配置、文档
2. **预估变更类型分布**：新增、重构、测试补充、文档更新
3. **高 PR 规模风险**：如果预估超过 15 个文件，标注风险并建议拆分策略

示例：
```markdown
## PR 规模预估

- 预估变更文件数量：约 N 个（源码 X、测试 Y、配置 Z、文档 W）
- 预估变更类型：<类型分布>
- PR 规模风险评估：<低于/超出> 15 文件阈值，<单一 PR 足够 / 建议拆分>
```
...
风险描述应回答：风险是什么、最可能影响哪里、如何提前验证、失败时如何收缩范围。

### Pre-Condition Markers (REQUIRED:BEFORE_CODING)

Plans MUST annotate mandatory pre-conditions using the `REQUIRED:BEFORE_CODING` marker.

**Format**:
- **REQUIRED:BEFORE_CODING** `<description>` — Verify: `<command>` — If not met: `<action>`

**Rules**:
- Only mark truly mandatory pre-conditions
- Each marker MUST include a concrete verification command and failure action
- Typical triggers: rebase needed, dependency not yet available, environment variable missing
- Place markers in "Risks & Considerations" as "Mandatory Pre-Conditions"

如果在规划过程中发现额外 bug、疑点或待跟进事项：
- 用 handoff append 单独记录
- 不要混入计划步骤，除非它们已被纳入当前明确范围
...
- 必须列出：直接修改文件对应的测试、re-export 或间接引用的测试、集成测试
- 必须说明：为什么窄验证足够覆盖风险
- 项目专属测试路径约定见 plan.policy@project

**测试范围覆盖检查**：
Planner 在指定定向测试范围时，必须检查：直接修改文件对应的测试、re-export 或间接引用的测试、集成测试、公开入口或关键路径的集成测试。
