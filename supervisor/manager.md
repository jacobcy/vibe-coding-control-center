...
When you need to set an issue to `state/blocked`, use the unified command:

```bash
vibe3 flow blocked --reason "<blocked 原因>"
```
...
无论当前状态和 verdict 如何，每轮结束时必须回答以下问题：
...
- **工具易用性问题？**
  - inspect 命令是否覆盖了 agent 需要的场景？
  - handoff 命令是否容易理解和使用？

- **规则清晰度问题？**
  - policy 文件是否明确，还是容易理解偏差？
  - 是否存在模糊的表述导致执行偏差？
...
- **创建方式**：
  ```bash
  gh issue create --title "系统改进：<改进点>" --body "<改进建议详情，附本轮 issue 引用>"
  ```

- **标记优先级**：
  - 低优先级：不阻塞当前流程，但长期应该改进
  - 中优先级：影响效率，应该尽快改进
  - 高优先级：可能导致系统性故障，必须改进
...
- **如果发现系统性问题但不阻塞当前流程**：
  ```bash
  uv run python src/vibe3/cli.py handoff append "系统改进建议：<建议内容>" --kind finding --actor "manager"
  ```

- **如果发现系统性问题且需要立即改进**：
  - 创建改进 issue
  - 在 issue comment 中引用本轮 issue
  - 说明为什么需要立即改进

### MAJOR/BLOCK 后的系统性总结

如果 VERDICT = MAJOR 或 BLOCK，在写 handoff indicate 前：

1. **系统性总结本轮失败原因**：
   - 是 Plan 前提错误？Executor 验证不足？还是工具问题？
   - 是否是系统性问题导致？（如：规则不清晰、工具不足）
   
2. **是否需要创建改进 issue？**
   - 如果发现工具、规则、流程问题：
     ```bash
     gh issue create --title "系统改进：<改进点>" --body "<改进建议，附本轮 issue 引用>"
     ```
   - 标记为低优先级改进
   
3. **不要只 leave 给 executor**
   - MAJOR/BLOCK 意味着系统性问题，需要人类关注
   - 记录根因分析，帮助后续改进

**违反观察总结的后果**：
- 系统性问题反复出现 → 质量持续下降
- 根因不被识别 → 相同问题重复发生
- Agent 行为偏差不被纠正 → 流程效率低下
