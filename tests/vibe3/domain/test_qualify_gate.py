...
class TestQualifyGateGitHubClosed:
    """Tests for run_qualify_gate GitHub closed check (all states)."""

    def test_run_qualify_gate_closed_ready_issue(
        self, qualify_gate_service, mock_flow_manager
    ):
        """READY + CLOSED should return None and terminalize."""
        ...
        mock_flow_manager.get_flow_for_issue.return_value = {"branch": "task/issue-101"}
        ...
        with patch(
            ...
            assert result is None
            # Verify terminalization was called
            mock_cleanup_svc.assert_called_once()

    def test_run_qualify_gate_closed_claimed_issue(
        self, qualify_gate_service, mock_flow_manager
    ):
        """CLAIMED + CLOSED should return None and terminalize."""
        from unittest.mock import patch

        closed_issue = IssueInfo(
            number=102,
            title="Closed Claimed Issue",
            state=IssueState.CLAIMED,
            labels=["state/claimed"],
            github_state="CLOSED",
        )
        mock_flow_manager.get_flow_for_issue.return_value = {"branch": "task/issue-102"}
    ...
    def test_run_qualify_gate_closed_in_progress_issue(
        self, qualify_gate_service, mock_flow_manager
    ):
        """IN_PROGRESS + CLOSED should return None and terminalize."""
        from unittest.mock import patch

        closed_issue = IssueInfo(
            number=103,
            title="Closed In-Progress Issue",
            state=IssueState.IN_PROGRESS,
            labels=["state/in-progress"],
            github_state="CLOSED",
        )
        mock_flow_manager.get_flow_for_issue.return_value = {"branch": "task/issue-103"}
    ...
    def test_run_qualify_gate_open_issue_passes(
        self, qualify_gate_service, sample_issue, mock_config
    ):
        """OPEN issue should not trigger closed check, goes normal path."""
        ...
        # Should proceed to normal logic (return trigger_state if label matches)
        assert result == IssueState.READY

    def test_run_qualify_gate_none_github_state_passes(
        self, qualify_gate_service, sample_issue, mock_config
    ):
        """github_state=None should not trigger closed check."""
        issue_no_github_state = IssueInfo(
            number=105,
        ...
        # Should proceed to normal logic
        assert result == IssueState.READY
