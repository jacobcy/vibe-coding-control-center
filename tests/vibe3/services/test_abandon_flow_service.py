...
class TestAbandonFlowService:
    ...
    @pytest.fixture
    def abandon_service(self, mock_github, mock_pr_service, mock_flow_service):
    ...
    def test_manager_ready_abandon_closes_issue_and_aborts_flow(
        self, abandon_service, mock_github, mock_pr_service, mock_flow_service
    ):
        """Abandon from ready state closes issue and aborts flow."""
        mock_github.close_issue_if_open.return_value = "closed"
        ...
        mock_github.close_issue_if_open.assert_called_once_with(
    ...
    def test_abandon_continues_on_partial_failures(
        ...
        # Issue close fails
        mock_github.close_issue_if_open.side_effect = Exception("Issue API error")
        # PR close succeeds
        ...
        mock_github.close_issue_if_open.assert_called_once()
    ...
    def test_abandon_skips_reclosing_issue_and_reaborting_flow_when_already_settled(
        ...
        assert result["pr"] == 456
        assert result["flow"] == "aborted"
        mock_github.close_issue_if_open.assert_not_called()
        mock_pr_service.close_open_pr_for_flow.assert_called_once()
