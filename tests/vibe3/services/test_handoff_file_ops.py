...
@pytest.fixture
def handoff_storage(mock_git_client):
    """Create a HandoffStorage instance with mocked dependencies."""
    return HandoffStorage(git_client=mock_git_client)
...
class TestEnsureCurrentHandoff:
    ...
    def test_ensure_current_handoff_creates_template(
        ...
        handoff_path = handoff_storage.ensure_current_handoff()
    ...
    def test_ensure_current_handoff_existing_file_is_idempotent(
        ...
        handoff_path2 = handoff_storage.ensure_current_handoff()
    ...
    def test_ensure_current_handoff_force_overwrites_existing_file(
        ...
        overwritten_path = handoff_storage.ensure_current_handoff(force=True)
...
class TestReadCurrentHandoff:
    ...
    def test_read_current_handoff(self, handoff_storage, temp_git_dir, mock_git_client):
        """Test reading current handoff file."""
        mock_git_client.get_git_common_dir.return_value = str(temp_git_dir)

        handoff_storage.ensure_current_handoff()

        content = handoff_storage.read_current_handoff()
        assert "# Handoff: feature/test-branch" in content

    def test_read_current_handoff_not_found(self, handoff_storage):
        """Test reading non-existent handoff file raises error."""
        with pytest.raises(UserError):
            handoff_storage.read_current_handoff()
...
class TestClearHandoffForBranch:
    ...
    def test_clear_handoff_for_branch_removes_directory(
        ...
        removed_dir = handoff_storage.clear_handoff_for_branch("task/issue-329")
...
class TestAppendCurrentHandoff:
    ...
    def test_append_current_handoff_creates_file_if_missing(
        ...
        handoff_path = handoff_storage.append_current_handoff(
    ...
    def test_append_current_handoff_appends_to_updates_section(
        ...
        handoff_storage.append_current_handoff(
...
class TestGetHandoffDir:
    """Tests for get_handoff_dir method."""
    ...
    def test_get_handoff_dir_sanitizes_branch_name(
        ...
        handoff_dir = handoff_storage.get_handoff_dir()
