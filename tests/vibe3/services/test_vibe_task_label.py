...
def test_reclassify_from_task_to_dependency_removes_vibe_task_label(
    tmp_path, monkeypatch
):
    """Verify that reclassifying from task to dependency removes vibe-task label."""
    db_path = tmp_path / "test.db"
    store = SQLiteClient(db_path=str(db_path))
    service = TaskService(store=store)
    ...
    # ACT: reclassify from task to dependency
    service.reclassify_issue(
        "task/test-branch",
        999,
        old_role="task",
...
def test_reclassify_from_dependency_to_task_adds_vibe_task_label(tmp_path, monkeypatch):
    """Verify that reclassifying from dependency to task adds vibe-task label."""
    db_path = tmp_path / "test.db"
    store = SQLiteClient(db_path=str(db_path))
    service = TaskService(store=store)
    ...
    # Setup: create flow with dependency issue
    store.update_flow_state("task/test-branch", flow_slug="test")
    service.link_issue("task/test-branch", 888, role="dependency")

    # Reset mock to check the reclassify call
    mock_label_service.reset_mock()

    # ACT: reclassify from dependency to task
    service.reclassify_issue(
        "task/test-branch",
        888,
        old_role="dependency",
    ...
    # ASSERT: confirm_vibe_task was called with should_exist=True
    mock_label_service.confirm_vibe_task.assert_called_once_with(888, should_exist=True)


def test_reclassify_between_related_and_dependency_no_label_change(
    tmp_path, monkeypatch
):
    """Verify reclassify between related and dependency does NOT change label."""
    db_path = tmp_path / "test.db"
    store = SQLiteClient(db_path=str(db_path))
    service = TaskService(store=store)
    ...
    # Setup: create flow with related issue
    store.update_flow_state("task/test-branch", flow_slug="test")
    service.link_issue("task/test-branch", 777, role="related")

    # Reset mock to check the reclassify call
    mock_label_service.reset_mock()

    # ACT: reclassify from related to dependency
    service.reclassify_issue(
        "task/test-branch",
        777,
        old_role="related",
