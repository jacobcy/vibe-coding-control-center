...
class TestDispatchSafetyGap:
    ...
    the active-session-gate, and preflight. After C2 they flow through like
    any other entry so remote intervention is detected and finished flows can
    be re-dispatched to their next phase.
    """

    def test_inprogress_entry_kicked_on_non_manager_assignee(
        self, mock_coordinator: GlobalDispatchCoordinator
    ) -> None:
        ...
        mock_coordinator._load_issue = lambda n: IssueInfo(
            number=42,
            title="t",
        ...
        dispatched = mock_coordinator._dispatch_loop(tick_id=1)
    ...
    def test_inprogress_entry_reaches_preflight_when_no_session(
        ...
        mock_coordinator._run_dispatch_preflight = (
            lambda issue: DispatchPreflightDecision(
    ...
    def test_inprogress_entry_skipped_when_live_session(
        ...
        mock_coordinator._registry = registry
