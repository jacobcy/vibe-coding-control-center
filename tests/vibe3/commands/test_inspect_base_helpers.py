...
def test_validate_base_branch_exists():
...
def test_validate_base_branch_resolved_via_remotes():
...
def test_validate_base_branch_not_found():
...
def test_count_changed_lines_in_code_paths(mock_count, mock_get_config):
...
def test_build_json_output(
    mock_score, mock_dag, mock_collect, mock_serena, mock_get_config
):
...
@patch("typer.echo")
def test_print_human_output(mock_echo):
