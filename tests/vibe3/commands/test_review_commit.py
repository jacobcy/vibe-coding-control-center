"""Tests for vibe review commit subcommand.
...
def test_review_commit_missing_arg_shows_error():
    result = runner.invoke(app, ["commit"])
    assert result.exit_code != 0
    assert "missing" in result.output.lower() or "error" in result.output.lower()


def test_review_commit_pass():
    patches = _patch_review_deps()
    with patches[0], patches[1], patches[2], patches[3], patches[4]:
        result = runner.invoke(app, ["commit", "HEAD~1"])
    assert result.exit_code == 0


def test_review_commit_help():
