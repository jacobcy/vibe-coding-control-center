...
def test_get_recent_commits_success():
    ...
    with (
        patch(f"{PR_ANALYSIS}.subprocess.run") as mock_run,
...
def test_get_recent_commits_empty():
    ...
    with patch(f"{PR_ANALYSIS}.subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(
            returncode=0,
            stdout="\n",
        )
...
def mock_config():
    ...
    with patch(f"{PR_ANALYSIS}.get_config") as mock:
        config = MagicMock()
...
def mock_generate_score():
    ...
    with patch(f"{PR_ANALYSIS}.generate_score_report") as mock:
        mock.return_value = {
            "score": 6,
            "level": "MEDIUM",
            "block": False,
