...
async def test_pool_exhaustion_counter_increments(monkeypatch) -> None:
    ...
    server = HeartbeatServer(
        OrchestraConfig(
            ...
            exhaustion_threshold_ticks=10,  # Set high so it won't stop early
        ),
    ...
    async def _sleep_once(_seconds: float) -> None:
        nonlocal tick_count
        tick_count += 1
        if tick_count >= 6:  # Run 6 sleeps to allow 5 complete ticks
            server.stop()
    ...
    # Counter should have incremented to 5 (after 5 complete ticks)
    assert server._exhausted_ticks == 5
    # Should have exhaustion events
...
async def test_pool_exhaustion_disabled_does_not_check(monkeypatch) -> None:
    ...
    assert server._exhausted_ticks == 0
    # Should NOT have exhaustion events
    assert not any("pool exhausted" in e for e in events)
    # Server should still be running until manual stop
    assert tick_count == 5
