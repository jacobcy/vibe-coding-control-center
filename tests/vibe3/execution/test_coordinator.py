...
def test_coordinator_dispatch_success(mock_dependencies):
    ...
    backend.start_async_command.assert_called_once_with(
        ["echo", "hello"],
        execution_name="vibe3-planner-issue-42",
        cwd=Path("/tmp/wt"),
        env={"FOO": "bar"},
...
def test_coordinator_dispatch_launch_fails(mock_dependencies):
    ...
    # Setup backend to raise exception
    backend.start_async_command.side_effect = Exception("Tmux failed to start")
