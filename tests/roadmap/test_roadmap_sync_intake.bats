...
@test "roadmap: sync_issue_intake_candidates intakes both vibe-task issues and merged PRs (when opt-in)" {
  ...
  cat > "$fixture/vibe/roadmap.json" <<'JSON'
{
  ...
  "items": [
    ...
    {
      "roadmap_item_id": "mirrored-via-alt-ref",
      "title": "Mirrored via alt ref",
      "source_refs": ["gh:owner/repo#303"],
      "issue_refs": ["gh:owner/repo#303"],
      "github_project_item_id": "PVTI_issue_303"
    }
  ]
}
JSON

  run env FIXTURE="$fixture" VIBE_ROADMAP_SYNC_INTAKE_PRS=1 zsh -c '
...
[
  ...
  {"id":"ISSUE_303","number":303,"title":"Issue with alt ref","url":"https://github.com/owner/repo/issues/303"}
]
JSON
    }
...
JSON
  ...
  # Ensure mirrored items were NOT added again
  [ "$(grep -c "ISSUE_90" "$add_log")" -eq 0 ]
  [ "$(grep -c "ISSUE_303" "$add_log")" -eq 0 ]
}

@test "roadmap: sync_issue_intake_candidates skips PR intake when not opt-in" {
  local fixture; fixture="$(mktemp -d)"
  local add_log; add_log="$(mktemp)"
  mkdir -p "$fixture/vibe"
  echo '{"schema_version":"v2","project_id":"PVT_project","items":[]}' > "$fixture/vibe/roadmap.json"

  run env FIXTURE="$fixture" VIBE_ROADMAP_SYNC_INTAKE_PRS=0 zsh -c '
    source "$VIBE_LIB/config.sh" 2>/dev/null || true
    source "$VIBE_LIB/utils.sh" 2>/dev/null || true
    source "$VIBE_LIB/roadmap_query.sh"
    source "$VIBE_LIB/roadmap_write.sh"
