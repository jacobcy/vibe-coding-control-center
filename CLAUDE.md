...
- `vibe3 task status` — 查看全局任务面板
- `vibe3 task show` — 查看当前任务详情（可接 issue 编号）
- `vibe3 handoff status` — 查看当前 flow 的 handoff 链和最近记录
- `vibe3 handoff show @plan` — 查看 plan artifact（`@current` 查看当前 handoff）
- `vibe3 snapshot show` — 查看项目健康度度量仪表盘
- `vibe3 serve status` — 查看 Orchestra 服务状态、FailedGate 及近期活动
- `vibe3 serve start` — 启动 Orchestra 服务
- `vibe3 serve stop` — 停止 Orchestra 服务
...
角色间的权力边界与制衡关系见 [docs/standards/vibe3-role-checks-and-balances-standard.md](docs/standards/vibe3-role-checks-and-balances-standard.md)。
...
- `supervisor/policies/` 定义“我现在处于什么执行模式”。
