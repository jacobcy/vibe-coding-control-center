...
- `wtrm <worktree> [--force]` - Remove worktree with confirmation

**Tmux Session Management**:
- `tmnew <task-slug> [agent]` - Create tmux session with auto-naming
...
- `tmkill <session> [--force]` - Kill session with confirmation
...
- `skill_prepare_environment <task> [agent] [base]` - Create complete environment
- `skill_cleanup_environment <task> [agent]` - Remove complete environment

## 目录职责
- `bin/`: CLI 分发入口
- `lib/`: Shell 核心逻辑
- `config/`: keys 与 aliases
  - `config/aliases/worktree.sh`: Worktree 管理命令（V3 Execution Plane）
  - `config/aliases/tmux.sh`: Tmux 会话管理（V3 Execution Plane）
  - `config/aliases/execution-contract.sh`: 执行结果契约（V3 Execution Plane）
  - `config/aliases/session-recovery.sh`: 会话恢复命令（V3 Execution Plane）
- `skills/`: 治理与流程技能
  - `skills/execution-plane/`: V3 Execution Plane 技能（OpenClaw 自动化）
- `.agent/`: rules/context/workflows
  - `.agent/execution-results/`: 执行结果存储（V3）
  - `.agent/recovery-history.log`: 恢复历史日志（V3）

## HARD RULES

### 代码规模与质量 (LOC & Quality)
1.  **LOC 限制**: `lib/ + bin/` 总行数 <= 1800。单文件 <= 200 行。
