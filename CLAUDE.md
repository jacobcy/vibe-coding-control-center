...
3. **Tier 1 (Shell 能力层): Shell Commands & Aliases**
   - 核心命令组（`bin/vibe check/tool/keys`）与底层 Alias 组（`wtnew`, `vup`）；现场确认与最小绑定操作由 V3 `vibe3` 命令承担。
...
- `dev/issue-<id>`：人机协作分支。默认通过 `skills/` 中的入口协作推进，例如 `/vibe-new`、`/vibe-start`。
...
详细模式见 [.claude/rules/patterns.md](.claude/rules/patterns.md)。
...
- **[.claude/rules/kiro-integration.md](.claude/rules/kiro-integration.md)** — Kiro 集成规则
