...
- 详细用法见 [.agent/rules/cli-usage.md](.agent/rules/cli-usage.md)

**代码搜索工具选择（CRITICAL）**：
- **符号引用** → 用 `vibe3 inspect symbols`（精确到行号）
- **语义理解** → 用 `mcp__auggie__codebase-retrieval`（理解意图）
- **字符串匹配** → 用 `Grep`（字面量、配置项）
- 详细选择规则见 [.agent/rules/cli-usage.md](.agent/rules/cli-usage.md) 的"工具选择优先级"
...
- **[.agent/rules/patterns.md](.agent/rules/patterns.md)** — 执行模式与报告模式
- **[.agent/rules/cli-usage.md](.agent/rules/cli-usage.md)** — CLI 使用指南与工具选择
>>>>>>> 1f66736 (refactor: split large files to meet LOC limits)
