...
5. **Git 纪律**：不直接在 `main` 上开发；Git/worktree 生命周期以 [git-workflow-standard.md](docs/standards/git-workflow-standard.md) 为准。
6. **调用面显式标注**：文档和沟通中首次提及 `vibe` 能力时，必须区分 `shell` 与 `skill`，例如 `vibe flow (shell)`、`/vibe-save (skill)`。
7. **handoff 不是真源**：`.agent/context/task.md` 只作本地 handoff；读取后必须先核查共享真源与 git 现场，若发现不一致必须修正。正式规则见 [docs/standards/handoff-governance-standard.md](docs/standards/handoff-governance-standard.md)。
