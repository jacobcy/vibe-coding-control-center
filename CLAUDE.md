...
- `src/vibe3/`: Python 实现（V3），包含以下子模块：
  ...
  - `agents/` - AI Agent 执行层
    - `execution_role_policy.py` - Agent 执行策略（backend、prompt、session、concurrency 统一解析）
    - `execution_lifecycle.py` - Agent 生命周期管理（started/completed/failed 记录）
  ...
  - `services/` - 核心业务逻辑
    - `capacity_service.py` - 统一容量控制（所有执行角色）
    - `flow_service.py`, `task_service.py` - Flow/Task 业务逻辑
    - `pr_service.py`, `handoff_service.py` - PR/Handoff 业务逻辑
...
Vibe 3.0 提供了统一的基础设施服务，用于支持所有执行角色（manager/planner/executor/reviewer）：

### 核心服务

1. **ExecutionRolePolicyService** - 统一的执行配置解析
   - 解析 backend、prompt template、session strategy
   - 支持所有执行角色的配置解析
   - 避免配置解析逻辑重复

2. **CapacityService** - 统一的容量控制
   - 解决双层节流问题（StateLabelDispatchService vs ManagerExecutor）
   - 结合 live session count 和 in-flight dispatch tracking
   - 支持按角色配置不同容量

3. **ExecutionLifecycleService** - 统一的生命周期管理
   - 记录 started/completed/failed 事件
   - 自动更新 flow state（planner_status、executor_status 等）
   - 提供一致的生命周期事件记录

4. **BackendProtocol** - Protocol-based dependency injection
   - 定义 Backend 接口
   - 支持测试时注入 mock backend
   - 便于扩展新的 backend 实现
