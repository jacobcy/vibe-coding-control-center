---
name: vibe-onboard
description: Use after `scripts/install.sh` to guide users through machine-level setup, dependency checks, Claude/Codex toolchain readiness, and core feature introduction. Interactive onboarding for new users. Delegates diagnostics to `vibe doctor` and `vibe keys check`. Do not use for project-level configuration (use vibe-project-check instead).
---
...
仅在 `scripts/install.sh` 之后进行机器级安装检查和首次使用引导；目标 repo 配置检查转交 `vibe-project-check`。
...
- **负责**：安装后的机器级检查；`vibe` 命令可用性；`vibe doctor` / `vibe keys check`；Claude/Codex 外部工具链 readiness；已知上游兼容问题说明
- **不负责**：某个 repo 的 `.vibe/` 配置补全、labels、scene_base_ref、prompt readiness、skills inventory 审计

分流规则：

- 当前项目 / repo 的工具与环境检查 → `skills/vibe-project-check/SKILL.md`
- skills 安装、symlink、冗余、全局/项目级对齐 → `skills/vibe-skills-manager/SKILL.md`
- 仅需项目导览 / 命令索引 → `skills/vibe-instruction/SKILL.md`

前提：

- 命令链路已经由 `zsh scripts/install.sh` 建好；如果 `vibe` 命令还不可用，先完成安装并重新加载 shell
- 当前项目 / worktree 已执行 `zsh scripts/init.sh`；通过 `wtnew`、创建新分支或 V3 自动创建的新 worktree 通常会自动跑一次，但如果还没做项目初始化，先执行它再进入 onboarding
...
- `/vibe-onboard` 不直接读取 `config/v3/dependencies.toml`，也不重复实现底层检查逻辑
- Claude / Codex 外部工具链、hooks、MCP、claude-mem 已知兼容性与本机规避策略，以 `docs/standards/plugin-setup-standard.md` 为真源
...
/vibe-onboard
  - Step 0: 收集底层事实
      ...
      - 如用户关心 Claude/Codex 工具链、hooks、MCP、claude-mem、安装兼容性，先读取 `docs/standards/plugin-setup-standard.md`
  ...
  - Step 5: skills 体系引导
      ...
      - 引导用户理解三类体系：
          ...
          - OpenSpec：本项目不推荐，统一用 spec-kit（见 docs/standards/spec-kit-workflow-standard.md）
...
- `vibe doctor`
- `vibe keys check`
- `docs/standards/plugin-setup-standard.md`（Claude/Codex toolchain 真源）

如果这些命令的结果和文档描述冲突，以命令输出为准。`config/v3/dependencies.toml` 属于 `doctor` / `keys` 的底层真源，不属于 `/vibe-onboard` 直接消费的接口。
