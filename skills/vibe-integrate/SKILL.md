...
当 `bash scripts/hooks/check-python-loc.sh` 报告 Python 总量触及或超过 `config/settings.yaml` 中的 `total_file_loc.v3_python` 阈值时，**必须**先开展一轮死代码审计，而不是直接要求提升阈值。
...
将文件加入 `config/settings.yaml` 的 exception 清单时，必须遵守容量约束：
