---
name: vibe-save
description: Use when the user wants to save session context, says /vibe-save, or when ending a session and you want to preserve work state. Use vibe3 handoff to record the current state.
---
...
- Shell 可以调用 `git` / `gh` / worktree 动作，但 skill 不得把这些机械步骤改写成"自动判断"或"自动修复"。
...
- 不得把 task issue 绑定关系改写成 GitHub 官方层身份。
...
handoff 应优先回答"下个会话接手时需要知道什么"，而不是扩展成认知档案库。
...
- 若 task 状态在本会话已明确，使用 `uv run python src/vibe3/cli.py flow show` 确认状态。
