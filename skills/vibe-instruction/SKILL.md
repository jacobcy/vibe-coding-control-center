...
uv run python src/vibe3/cli.py task resume 456        # 等价于 --label auto，只恢复 blocked，不删除现场
...
- `task resume` 用于清除 blocked 状态并推断恢复 label，不删除现场。
...
- **其他 auto-task scene 需要恢复**：运行 `vibe3 task resume <issue-number> --label auto`（禁止使用 `--blocked`，会恢复所有 blocked issues）。
