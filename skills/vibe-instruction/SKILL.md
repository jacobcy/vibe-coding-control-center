...
- 注意：CLI 有一个隐藏的顶层 `status` 命令（兼容性保留），实际等同于 `task status`。
...
- 需要删除 flow/worktree 时使用 `flow rebuild`，不要用 `task resume` 隐式触发。
...
```bash
vibe3 mcp run  # 在 stdio 模式下运行 MCP 服务器（用于 MCP 客户端集成）
```
...
uv run python src/vibe3/cli.py run --plan         # 派发执行（默认异步）
...
- **恢复特定 blocked issue**：运行 `vibe3 task resume <issue-number> --label auto`。
- **批量恢复 blocked issues**：运行 `vibe3 task resume --blocked`（谨慎使用，会恢复所有 blocked issues）。
