...
- orphan auto task scene 需要 `flow rebuild` 或 cleanup 回到可恢复起点
...
- 当前目录承载的 flow 就是目标 task 的确定性现场
  - 修复命令：仅限当前 `vibe3 check` 默认模式已经明确支持的最小修复
- 其他 auto task flow 需要恢复 blocked 状态
  - 修复命令：`uv run python src/vibe3/cli.py task resume <issue-number> --label auto ...`
...
- 要不要重建其他 issue 的 auto task scene
  - shell 有 `flow rebuild` 能力，但需要用户确认具体 issue
...
- `uv run python src/vibe3/cli.py task resume`：只清除 blocked 状态并恢复 label，不删除现场
