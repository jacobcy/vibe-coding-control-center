...
若要核对当前现场与全局状态，可额外读取 `uv run python src/vibe3/cli.py flow show` 或 `uv run python src/vibe3/cli.py task status --all --check`。
...
- `uv run python src/vibe3/cli.py flow show` / `uv run python src/vibe3/cli.py task status --all --check`：读取当前现场与全局状态
