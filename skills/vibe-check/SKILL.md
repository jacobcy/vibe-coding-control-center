...
| 检查项 | 说明 | 严重程度 |
|--------|------|----------|
| 幽灵分支 | 是否存在无关联任务的长期存活分支 | 高 |
| 文档散落 | 任务文档是否遗留在 docs/plans 或 docs/prds | 高 |
| 文件存在 | 引用的文件是否存在 | 高 |
| 路径正确 | 文件路径是否正确 | 高 |
...
## 改进流转协议 (Remediation Protocol)

1. **针对文档散落**: 
   - 识别文档所属的 Task ID（通过文件名日期或内容）。
   - 如果 `docs/tasks/{id}/` 不存在，执行 `mkdir -p`。
   - 使用 `mv` 将散落文档移入规范目录。
2. **针对幽灵分支**:
   - 检查该分支的最后提交时间 (`git log -1`)。
   - 如果超过 7 天无更新且无对应任务，建议用户删除。
   - 如果用户确认，执行 `git branch -D`。
...
5. **审计前置** - 在执行 check 或 commit 前，运行 `/review` 来进行深度 AI 审计。
