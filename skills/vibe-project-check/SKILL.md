...
**验证**: `ls ~/.vibe/src/vibe3/resources/` 显示文件；所有 9 个路径均存在。
...
**Step 8.4: internal manager prompt readiness（已知 gap）**

```bash
# 明确标注 internal manager 的已知限制
echo "GAP: internal manager 命令缺少 --dry-run/--show-prompt CLI 参数（源码硬编码 dry_run=False, show_prompt=False）。无法在不执行 agent 的前提下验证 prompt readiness。需待底层 issue 补齐此能力后再加入检查。"
```
...
- `internal manager` gap → "已知限制：`vibe3 internal manager` 缺少 dry-run/show-prompt 选项，无法非破坏性验证 manager prompt readiness"

**验证**: `plan`, `run`, `review` 均产生 prompt 输出；`internal manager` gap 已明确记录。
...
✅ Global runtime assets: 9/9 present
✅ Target repo init: .vibe/config.yaml valid, profile=github-flow
✅ Agent/toolchain: all essential tools available
⚠️ Prompt readiness: plan/run/review ok; internal manager gap (known limitation)
...
- PROMPT_READINESS_GAP → "等待底层 command issue 修复"
...
- Issue #1905: internal manager dry-run/show-prompt (partially resolved)
