...
# Three-layer governance labels (write-side; see docs/governance/governance-roadmap-closed-loop.md)
sync_label "orchestra-scanned" "Passed governance observer 审查" "FF9933"
sync_label "orchestra-governed" "Assignee-pool decided (governed)" "9933FF"
sync_label "roadmap-reviewed" "Passed roadmap decider decision" "CC99FF"
# Legacy `orchestra` (umbrella) label is no longer synced here.
# sync-labels.sh is non-destructive — existing issues may still carry it.
# Filtering compatibility lives in src/vibe3/roles/governance_utils.py
# (build_broader_repo_entries skips legacy `orchestra` alongside the new labels).
# To audit remaining legacy-labeled issues:
#   gh issue list --label orchestra --state all --json number,title
