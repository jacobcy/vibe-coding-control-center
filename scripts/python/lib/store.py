...
class Vibe3Store:
    ...
    def update_flow_state(self, branch, **kwargs):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("INSERT OR IGNORE INTO flow_state (branch, flow_slug, updated_at) VALUES (?, ?, ?)", 
    ...
    def add_issue_link(self, branch, issue_number, role):
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT OR REPLACE INTO flow_issue_links (branch, issue_number, issue_role, created_at)
                VALUES (?, ?, ?, ?)
