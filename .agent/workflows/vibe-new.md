...
- `/vibe-new` 当前支持两种 shell 路径：
  ...
  - 当前目录切到下一个逻辑 flow：通过 `vibe task add` / `vibe task update` 准备任务元数据，再调用 `vibe flow new <feature> --agent <agent> --branch <ref>` 创建新的 flow / branch。
...
6. **Checkpoint Output & HARD STOP**
  ...
  - 回复用户："✍️ 规划文件 `plan.md` 已就绪。执行引擎已被挂起。请您审查图纸，若无异议，请回复 `/vibe-start` 唤醒 Execution 机器人开始编码；如需在当前目录创建或绑定 flow，请使用 `vibe flow new <feature> --agent <agent> --branch <ref>` 或 `vibe flow bind <task-id> --agent <agent>`。若当前有未提交改动且要带入新 flow，请执行 `vibe flow new <feature> --agent <agent> --branch <ref> --save-unstash`；若需要新的物理 worktree，请使用 `wtnew` / `vnew`。"
