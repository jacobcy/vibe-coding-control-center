...
| ADR | 为什么 / 决策了什么 | 不可变、可 supersede | `docs/decisions/`（新增） | 人类决策时结晶 |
...
> "standards 由 supervisor 动态维护"是目标态（现状治理材料 intake/pool/cron 并不维护 standards）。无论是否实现，设计含义一致：**ADR 必须独立于 living 区，因为它不可变**。让 standards 真正 supervisor-maintained 列为独立 follow-up，不在本轮。
...
- ADR 写**为什么 + 决策了什么**（不可变）；Standards/design/philosophy 写**怎么做**（可变）；RFC 是 GitHub 信号，不入库。
...
### 3. 生成侧：RFC → ADR 结晶（路由进管线，skill 不写文件）

矛盾：vibe-task/vibe-roadmap 是 **label-only / 不写文件**，而 ADR 是文件、必须 PR 落地。解法**不是**给 skill 加写文件能力，而是**复用现有 issue → flow → PR 管线**：

> 架构级 rfc 决策 = "采纳并结晶为 ADR-NNNN"：在 INDEX 预留号（`status: proposed`）+ 写 `[decision]` 引用 `ADR-NNNN` → 给 issue 设 **`state/ready`**，让它**流入正常执行管线**（vibe-new / orchestra → 分支 → 写 ADR 文件 + 更新 INDEX → PR → 合并 → ADR 转 `accepted`、rfc 真正 resolved）。

- 架构级判据（三条全满足才结晶）：① 跨任务/跨模块的架构选型；② 有真实权衡或反直觉；③ 期望跨 PR/issue 长期有效。
- 非架构级 rfc（绝大多数）→ 维持 `[decision]` 评论，**不产 ADR，流程不变**。
- 效果：skill 仍只打 label + 写评论（保持纯洁），但 rfc 的"终点"从"悬浮在 GitHub"变成"**路由进执行管线、交付物是 ADR PR**"。零新机制。

挂载点：vibe-task `Step 2.2 方案1（采纳并推进）`、vibe-roadmap `rfc override / Step X` 分支。

### 4. 消费侧：plan / task / roadmap 决策时必查 ADR（纯 policy，无 CI 门禁）

- **`supervisor/policies/plan.md`**（plan mode 真源）：在"上下文圈定"阶段加一步——**计划前必查 `docs/decisions/INDEX`；新计划不得违反 `accepted` ADR；要偏离必须显式提议 supersede，而非偷偷违反**。
...
ADR 个体不可变，但 **supersede 链反映当前生效决策**——这正是 standards(living, 当前 HOW) 之外，ADR 反映"当前 WHY"的方式：

- 新 ADR `supersedes: N`；旧 ADR 转 `status: superseded` + `superseded_by: M`。
...
4. 组件 §3 的结晶约定 → 加入 vibe-task + vibe-roadmap。
...
- vibe-task / vibe-roadmap / supervisor/policies/plan.md 含生成与消费约定文字。
...
无。三个核心决策（核心目标=长期记忆+追溯；边界=薄决策层+链接；范围=立模型+落 ADR+RFC→ADR 约定）及两个细节（ADR-0001=dogfood、rfc 路由进管线）均已确认。
