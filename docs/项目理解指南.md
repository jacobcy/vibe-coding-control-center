# Vibe Coding Control Center - 项目理解指南

## 🎯 项目定位

这是一个 **AI 开发工具管理中心**，用于统一管理和配置多个 AI 编程助手工具。

### 核心功能
1. **统一安装和配置** Claude Code、OpenCode 等 AI 工具
2. **菜单驱动界面** 提供友好的交互式控制中心
3. **项目初始化** 快速创建带有最佳实践的新项目
4. **安全管理** API 密钥和环境变量
5. **MCP 集成** Model Context Protocol 支持（GitHub、搜索等）

## 📁 项目结构

```
vibe-coding-control-center/
│
├── scripts/                    # 核心脚本
│   ├── vibecoding.sh          # 主控制中心（入口）
│   ├── backup-project.sh      # 项目备份工具
│   └── ...
│
├── install/                    # 安装脚本
│   ├── install-claude.sh      # Claude Code 安装（官方）
│   ├── install-opencode.sh    # OpenCode 安装（中国可用）
│   └── init-project.sh        # 项目初始化
│
├── config/                     # 配置文件
│   ├── keys.env              # API 密钥（不提交到 git）
│   ├── keys.template.env     # 密钥模板（提交到 git）
│   └── aliases.sh            # 命令别名
│
├── lib/                        # 工具库
│   ├── utils.sh              # 共享工具函数
│   └── utils_secure.sh       # 安全工具函数
│
├── docs/                       # 文档
│   ├── getting-started.md    # 入门指南
│   ├── agents-guide.md       # Agent 使用指南
│   └── usage_advice.md       # 使用建议
│
└── README.md                   # 项目说明
```

## 🔑 核心概念

### 1. 双工具支持

项目支持两个 AI 编程助手：

#### Claude Code（官方，需要国际网络）
- **API 端点**: `https://api.anthropic.com`
- **模型**: `claude-3-5-sonnet-20241022`
- **认证**: Anthropic API Key（`sk-ant-` 开头）
- **适用**: 有国际网络访问的用户

#### OpenCode（中国可用）
- **API 端点**: `https://coding.dashscope.aliyuncs.com/apps/opencode`
- **模型**: `qwen3-coder-plus`（阿里云通义千问）
- **认证**: 阿里云 token（`sk-sp-` 开头）
- **适用**: 中国大陆用户

### 2. 环境变量配置

**重要**：统一使用 `ANTHROPIC_AUTH_TOKEN`，不要混用 `ANTHROPIC_API_KEY`，避免环境冲突。

所有配置通过环境变量管理，存储在 `config/keys.env`：

```bash
# Claude Code（官方）
ANTHROPIC_AUTH_TOKEN=sk-ant-xxxxx                 # 统一使用 AUTH_TOKEN
ANTHROPIC_BASE_URL=https://api.anthropic.com
ANTHROPIC_MODEL=claude-3-5-sonnet-20241022

# Claude Code（中国 - 阿里云代理）
ANTHROPIC_AUTH_TOKEN=sk-sp-xxxxx                  # 阿里云 token
ANTHROPIC_BASE_URL=https://coding.dashscope.aliyuncs.com/apps/anthropic
ANTHROPIC_MODEL=qwen3-coder-plus

# OpenCode（原生支持多模型，无需配置）
# OpenCode 本身就支持 Qwen、DeepSeek、Moonshot 等模型
# 只需要配置对应的 API Key（可选）
DEEPSEEK_API_KEY=sk-xxxxx
MOONSHOT_API_KEY=sk-xxxxx

# 其他 API 密钥
GITHUB_PERSONAL_ACCESS_TOKEN=github_pat_xxxxx
BRAVE_API_KEY=BSAwxxxxx
```

### 3. 命令别名

安装后可用的快捷命令：

```bash
# Claude Code
c              # 启动 Claude 交互模式
ca "问题"      # 快速提问
cp "目标"      # 创建计划
cr             # 审查代码

# OpenCode
o              # 启动 OpenCode 交互模式
oa "问题"      # 快速提问

# 控制中心
vibe           # 启动 Vibe Coding Control Center
```

## 🌏 真正的"中国模式"是什么？

### ❌ 不是什么

**不是**通过"模型映射"来实现的！之前的代码创建了一堆 `model_config.json` 配置文件，但这些文件根本不会被使用。

### ✅ 是什么

有两种方案：

#### 方案 1：Claude Code + 环境变量 hack（无奈之举）

因为在中国无法访问 Anthropic 官方 API，所以通过修改环境变量让 Claude Code 指向阿里云的兼容端点：

```bash
# 官方模式（需要国际网络）
ANTHROPIC_API_KEY=sk-ant-xxxxx
ANTHROPIC_BASE_URL=https://api.anthropic.com
ANTHROPIC_MODEL=claude-3-5-sonnet-20241022

# 中国模式（阿里云代理，无奈之举）
ANTHROPIC_API_KEY=sk-sp-xxxxx  # 阿里云 token
ANTHROPIC_BASE_URL=https://coding.dashscope.aliyuncs.com/apps/anthropic  # 阿里云兼容端点
ANTHROPIC_MODEL=qwen3-coder-plus  # 实际使用 Qwen 模型
```

**关键点**：
- 同一个 Claude Code CLI 工具
- 阿里云提供了兼容 Anthropic API 的端点
- 通过修改环境变量切换
- **这是 workaround，不是正道**

#### 方案 2：OpenCode（正道，原生支持）

OpenCode 是专门为中国环境设计的 AI 编程助手：

```bash
OPEN_CODE_AUTH_TOKEN=sk-sp-xxxxx
OPEN_CODE_BASE_URL=https://coding.dashscope.aliyuncs.com/apps/opencode
OPEN_CODE_MODEL=qwen3-coder-plus
```

**关键点**：
- 原生支持 Qwen、DeepSeek、Moonshot 等多种模型
- 不需要任何 hack 或 workaround
- 专为中国环境设计
- **这是正道**

## 🚀 使用场景

### 场景 1：国际用户（有国际网络访问）

```bash
# 安装 Claude Code
./install/install-claude.sh

# 配置 Anthropic API Key
# 编辑 config/keys.env:
ANTHROPIC_API_KEY=sk-ant-xxxxx
ANTHROPIC_BASE_URL=https://api.anthropic.com
ANTHROPIC_MODEL=claude-3-5-sonnet-20241022

# 使用
c "帮我写一个 Python 函数"
```

### 场景 2：中国用户 - 方案 A（使用 Claude Code + 阿里云代理）

```bash
# 安装 Claude Code
./install/install-claude.sh

# 配置阿里云 token（让 Claude Code 指向阿里云）
# 编辑 config/keys.env:
ANTHROPIC_API_KEY=sk-sp-xxxxx  # 阿里云 token
ANTHROPIC_BASE_URL=https://coding.dashscope.aliyuncs.com/apps/anthropic
ANTHROPIC_MODEL=qwen3-coder-plus

# 使用（同样的命令，但实际使用 Qwen 模型）
c "帮我写一个 Python 函数"
```

**注意**：这是无奈之举，因为无法访问 Anthropic 官方 API。

### 场景 3：中国用户 - 方案 B（使用 OpenCode，推荐）

```bash
# 安装 OpenCode
./install/install-opencode.sh

# 配置阿里云 token
# 编辑 config/keys.env:
OPEN_CODE_AUTH_TOKEN=sk-sp-xxxxx
OPEN_CODE_BASE_URL=https://coding.dashscope.aliyuncs.com/apps/opencode
OPEN_CODE_MODEL=qwen3-coder-plus

# 使用
o "帮我写一个 Python 函数"
```

**推荐**：OpenCode 原生支持多种中国模型，是正道。

## 📝 总结

### 项目的本质
- **工具管理器**：统一管理多个 AI 编程助手
- **配置中心**：集中管理 API 密钥和环境变量
- **快捷入口**：提供便捷的命令别名

### "中国模式"的本质

有两种实现方式：

#### 方式 1：Claude Code + 环境变量 hack（无奈之举）
- **原理**：修改 `ANTHROPIC_BASE_URL` 指向阿里云的 Anthropic 兼容端点
- **实质**：让 Claude Code 通过阿里云代理使用 Qwen 模型
- **评价**：这是 workaround，因为无法访问 Anthropic 官方 API

#### 方式 2：OpenCode（正道）
- **原理**：使用专门为中国环境设计的工具
- **实质**：原生支持 Qwen、DeepSeek、Moonshot 等多种模型
- **评价**：这是正道，不需要任何 hack

### 关键理解
1. **Claude Code** 是官方工具，在中国通过修改环境变量使用 Qwen（无奈之举）
2. **OpenCode** 是原生支持多模型的中国友好工具（正道）
3. 通过**环境变量**来配置，而不是什么"映射配置文件"
4. 之前那些 `model_config.json` 配置文件完全是垃圾，没有任何代码读取它们

这就是整个项目的真相！简单、直接、有效。

