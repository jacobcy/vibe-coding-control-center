...
4. 明确 sync 的双向边界：只同步 GitHub Project 官方镜像字段；本地执行桥接字段保留在共享真源；冲突 fail-fast。
5. 在实现前补合同测试，覆盖缺失 `project_id`、当前 repo 不可推导、远端创建成功后本地回填、以及本地执行字段不会被写回 GitHub。
...
- `vibe roadmap sync` 的目标从 repo issue import 明确收敛为 GitHub Project 双向同步
