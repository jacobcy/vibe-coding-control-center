...
```bash
ANTHROPIC_AUTH_TOKEN=sk-ant-xxxxxxxxxxxxxxxxxxxxxxxx
```

### 4. Agent 配置 (.agent/)
- **context/**: 存放 AI 记忆与上下文 (`memory.md`, `task.md`)
...
| 配置项 | 值 |
|--------|-----|
| API 端点 | `https://api.anthropic.com`（可切换为中转站 `https://api.bghunt.cn`） |
...
"中国模式"默认使用 Claude 中转站（api.bghunt.cn），并可搭配 OpenCode 作为多模型补充。
