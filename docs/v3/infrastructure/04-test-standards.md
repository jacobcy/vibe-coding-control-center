...
| Clients | 测试外部系统交互（Mock） | < 200 行 | < 40 行 | **必须** |
...
**测试代码应该简洁，不得超过以下限制**（测试文件限制独立于 [03-coding-standards.md](03-coding-standards.md) 的生产代码限制）：

```
单个测试文件 < 180 行（Services 层）
单个测试文件 < 80 行（Commands 层）
单个测试文件 < 200 行（Clients 层）
```
...
# 检查覆盖率阈值
uv run pytest tests/ --cov=src/vibe3 --cov-fail-under=80
```
...
- [ ] 测试文件 < 最大行数限制（Services: 180 行，Commands: 80 行，Clients: 200 行）
