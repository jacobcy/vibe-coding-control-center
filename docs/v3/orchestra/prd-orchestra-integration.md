...
- 行为：
  ...
  - 创建或复用 flow
  - 在 issue 分支对应 worktree 调用 manager 执行链（不切换 `serve` 进程分支）
  - 如目标分支暂不支持 `run --worktree`，自动降级兼容执行
...
- `issue_comment/*`（配置的 `manager_usernames` 指令）
...
manager 执行工作目录策略（当前实现）：
- 优先复用已有分支 worktree
- 缺失时自动创建 `.worktrees/issue-<number>`
- 避免切换守护进程当前分支
