...
### 1. Roadmap-intake / Vibe-roadmap 硬编码

**问题**：`roadmap-intake` 和 `vibe-roadmap` 命令目前硬编码使用 `vibe-manager-agent` 作为默认 assignee。

**影响**：governance 自动创建的 issue 会永远落在默认 bot 上，无法路由到多机配置的其他 bot。
