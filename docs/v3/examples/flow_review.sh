...
_flow_review_usage() {
  ...
  echo ""
  echo "Usage: ${CYAN}vibe3 flow status${NC} [options] [<pr-or-branch>|--branch <ref>]"
  echo ""
  ...
  echo ""
  echo "示例："
  echo "  ${CYAN}vibe3 flow status${NC}              # 查看当前分支的 PR 状态"
  echo "  ${CYAN}vibe3 flow status 42${NC}           # 查看 PR #42 的状态"
  echo "  ${CYAN}vibe3 flow status --local${NC}      # 本地审查（自动选择 LLM）"
  echo "  ${CYAN}vibe3 flow status --local=codex${NC}    # 强制使用 codex"
  echo "  ${CYAN}vibe3 flow status --json${NC}       # JSON 输出，含 reviewEvidence 摘要"
}
