...
**边界说明**:
- ~~真正的语义真源曾是 `docs/plans/2026-03-13-vibe3-parallel-rebuild-design.md`~~ (已不存在)
- ~~真正的实施真源曾是 `docs/plans/2026-03-13-vibe3-parallel-rebuild-plan.md`~~ (已不存在)
- 第三轮顺序执行入口仍是 `docs/v3/handoff/README.md`
- 本文件现作为 Phase 2 施工参考，上述真源文件已归档移除
...
7. **一致性检查**: `vibe3 check` 必须验证本地责任链与远端真源对齐
...
1. **实现最小 handoff store**
   - ~~文件: `lib3/flow/state.sh`~~ (已不存在，现使用 `src/vibe3/services/handoff_service.py`)
...
vibe3 flow show
vibe3 flow status
vibe3 flow update test-feature
vibe3 flow switch main
vibe3 check
```
...
**范围**:
- Task state management via flow bindings
- `task show` - 显示 task 详情
- `task status` - 显示 task 状态总览
- `task resume` - 恢复失败或阻塞的 task

**实现步骤**:

1. **实现 task state management**
   - 职责: 通过 flow bindings 管理 task 与 issue 的关联关系
   - 实现: 使用 `flow bind` 和 `flow_issue_links` 表建立关联
   - 真源仍是 GitHub issue / Project
...
vibe3 task show
vibe3 task status
vibe3 task resume <issue-number>
```
...
**范围**:
- `flow bind <task-id>` - 绑定 task issue 到 flow
...
```bash
# 测试命令
vibe3 pr draft --task 123 --spec-ref docs/plans/test.md
vibe3 pr show
vibe3 pr ready
vibe3 pr merge
```
...
- `pr review` - PR 审查
- Task management via flow commands
...
3. **实现 flow 级别的 task 管理**
   - 通过 flow 命令管理 task 状态
   - 写入 handoff store
...
```bash
# 测试命令
vibe3 flow freeze --by "#123"
vibe3 pr review --local
vibe3 flow update --status in_progress
vibe3 handoff report --agent codex --model gpt-5.4
```
