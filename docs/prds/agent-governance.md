...
- 让 flow completion 成为一次回顾和沉淀节点，而不只是结束动作
  (Note: `flow done` is a deprecated V2 command; V3 uses direct git/gh for branch lifecycle)
...
- 把 flow completion 作为 harness retrospective 节点
  (Note: `flow done` is deprecated; current V3 flow lifecycle managed via git/gh)
...
- 在 flow completion 和 supervisor 中加入 retrospective
  (Note: `flow done` is deprecated; retrospective should integrate with current V3 flow lifecycle)
