...
```
┌─────────────────────────────────────────────────────────────────┐
│              Cognitive / Governance Layer (Tier 3)               │
│                        人类主权区（立法层）                        │
│  ┌─────────┐    ┌─────────┐    ┌──────────────────┐            │
...
1. **立法与行政分离**：人类主导立法（Tier 3 - Cognitive），AI 负责行政（Tier 2 - Skill），CI 与独立 AI 负责执法（Tier 3 - Governance）。
...
## 架构总览 (3-Tier Model Mapping)

Vibe Guard 开发范式在 3-Tier 架构中的落位：

- **Tier 3 (Cognitive/Governance)**: 负责立法（PRD/Spec/Plan）与司法（Audit/Review/Orchestra）。这是人类主权区与治理判定层。
- **Tier 2 (Skill)**: 负责行政执行。AI Agent 调用特定的 Skill 模块完成 Test/Code 产出。
- **Tier 1 (Shell)**: 负责原子能力支撑。提供基础的 Git 操作、文件读写、AST 分析和 Vibe3 运行时 CLI。
