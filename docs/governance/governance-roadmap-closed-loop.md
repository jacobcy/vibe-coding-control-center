...
架构级 rfc 决策不应止步于评论。当决策满足 ADR 结晶条件时（跨任务/跨模块、有真实权衡、期望长期有效），应直接推动一个小型 ADR PR，结晶为 `docs/decisions/` 下的 ADR 文件，并在 RFC issue comment 中指向该 PR，确保决策理由在 PR/issue 关闭后仍可追溯。

生成路径：`roadmap/rfc` → 人类决策 → 小型 ADR PR → 结晶为 ADR → 更新 standards。
