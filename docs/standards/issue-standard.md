...
3. flow bind / flow_issue_links 记录本地绑定关系事实（而依赖与阻塞状态的权威真源归属 GitHub body 投影，由 reconcile 同步缓存；见 v3/blocked-dependency-reconciliation-standard.md）

副作用（Mirror / Side Effect）：
- vibe-task 与 state/blocked 标签由对账机制或自动化自动镜像。
- **非真源**：它们是信号，不是治理判定的依据，也不是 execution record 本体。
- **瞬态性**：仅表示“当前正在/曾被 flow 绑定管理或阻塞”的逻辑视图，不代表 issue 的永久属性。
```
...
```
✅ 本地执行角色真源：SQLite flow_issue_links.issue_role
   - 只定义本地 flow <-> issue 绑定中的角色
   - 只保存最小运行时绑定事实
   - 由 vibe3 flow bind 写入，并同步写入 GitHub body 投影真源

✅ 阻塞与依赖状态真源：GitHub Issue Body 托管投影
   - 包含 State / Blocked reason / Blocked by 字段
   - 本地 flow_state 与 flow_issue_links(role=dependency) 为对账重建的缓存
   - 见 [blocked-dependency-reconciliation-standard.md](v3/blocked-dependency-reconciliation-standard.md)

❌ GitHub vibe-task / state/blocked label (同步信号)
  - 用于 GitHub 视角过滤
  - 只能由对账机制或自动化根据真源镜像
  - 不是状态判决的真源
...
1. ✅ **不做第二真源**
   - SQLite `issue_role` 记录本地角色绑定，GitHub issue body 投影记录阻塞/依赖状态真源
   - GitHub label 只是镜像信号，绝非真源
