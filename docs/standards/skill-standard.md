...
正式触发与分流以 `docs/standards/skill-trigger-standard.md` 为准；个人查看用备忘见 `docs/references/skill-loop-memo.md`。
...
| 规划入口 | `vibe-new` | 在当前上下文中决定 intake 来源，补 plan，绑定 task，必要时准备逻辑 flow | 执行代码、直接进入实现 |
| 执行入口 | `vibe-start` | 执行当前 flow 已绑定且已有 execution spec 的 task | 创建 issue、创建 task、补 plan |
