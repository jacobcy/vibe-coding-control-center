...
**功能**：
- `flow add`：在当前分支注册 flow
- `flow create`：创建新分支并注册 flow
- 两者对 `task/spec` 的绑定编排保持一致
...
# 2. 注册并绑定 task/spec
vibe3 flow add my-feature --task 220 --spec 220

# 3. 创建新分支并注册 flow
vibe3 flow create reports-storage --task 221 --base main
```
...
# 解除阻塞（切换回 active）
vibe3 flow switch <target-flow>
# 或继续在当前 flow 执行 run/review 流程
```
...
```bash
# 1. 创建 flow 并绑定 issue
vibe3 flow create flow-status-transitions --task 220
# → 创建分支 task/flow-status-transitions
# → 绑定 #220 为 task issue
# → 初始化 flow_state 记录

# 或指定自定义名称
vibe3 flow create my-feature --task 220
# → 创建分支 task/my-feature

# 2. 为当前 flow 记录补充 issue 关系（统一入口）
...
```bash
# task #221
vibe3 flow create reports-unified-storage --task 221
...
# task #222
vibe3 flow create pr-show-complete --task 222
...
Issue 关系统一通过 `flow bind` 建立。参数规范详见 [vibe3-command-standard.md](../standards/vibe3-command-standard.md)。
...
Issue 与 Flow 的关系只使用 `flow bind`：

| 命令 | 用途 | 支持的 role |
|------|------|------------|
| `flow bind` | 绑定或更换 task issue，或补充 related/dependency | task, related, dependency |
...
### 远端 GitHub Project 状态

目标流程：由 flow 生命周期自动联动远端 Project 状态，不再依赖手动 `task status`。
...
```bash
# 绑定主任务
vibe3 flow bind 221 --role task

# 绑定依赖
vibe3 flow bind 218 --role dependency
```

当 role 为 `task/dependency` 时，系统会自动尝试 project bridge 与标签镜像。
...
```bash
# 1. 创建新 flow
vibe3 flow create feature-a --task 220
# → 分支：task/feature-a
# → 绑定 issue #220

# 2. 开发过程中切换到另一个 flow
vibe3 flow create feature-b --task 221
# → 分支：task/feature-b
# → 绑定 issue #221
