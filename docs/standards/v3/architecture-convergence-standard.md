...
同时，架构决策机制已进入 **ADR 驱动模式** (Ref: [ADR-0001](../../decisions/0001-adopt-adr-loop.md))，形成 RFC → ADR → Standards 的闭环，确保架构演变理由可追溯。

但当前仓库仍处于“新旧并存”的迁移态：

- 旧的 `orchestra/services` 仍保留较多编排权，但已承认 `issue/`、`task/`、`handoff/` 领域子包的拆分正在进行中（Phase 2），语义重心正向领域子包收拢。
- `manager` 仍承担了部分执行控制面
- 多条角色链路仍各自拼装启动流程
...
- `ExecutionLifecycleService`：统一 started / completed / crashed (execution_failed) 记账
...
- **Protocol-based DI** (Ref: [ADR-0002](../../decisions/0002-protocol-based-di.md))：作为服务间解耦的技术手段，确保 services 层不直接依赖 agents 实现。
...
- 事件语义与处理规则：见 [event-driven-standard.md](event-driven-standard.md)
- runtime driver / tick / async child：见 [../vibe3-orchestra-runtime-standard.md](../vibe3-orchestra-runtime-standard.md)
- 状态机与 authoritative refs：见 [command-standard.md](command-standard.md)
- 模块职责边界与分层定义：见 [../v3-module-architecture-standard.md](../v3-module-architecture-standard.md)
- worktree 分配与 runtime session：见 [worktree-ownership-standard.md](worktree-ownership-standard.md)
...
### 2026-06-07: Architecture Standard Alignment (Issue #2332)

**Completed:**
- Updated "Background Judgment" to include ADR Loop (ADR-0001).
- Acknowledged modularity progress in domain subpackages (issue/, task/, handoff/) as part of Phase 2.
- Integrated Protocol-based DI (ADR-0002) as a core decoupling technique.

**Architecture Impact:**
- Documentation aligned with current decision-making mechanism (ADR-driven).
- Domain subpackage ownership recognized, shifting weight from legacy orchestration.
- Dependency inversion (DIP) reinforced via protocol-based injection.
