...
title: Python Capability Design
status: approved
scope: python-boundary
authority:
...
# Python Capability Design

本文档定义 Vibe Python CLI 的设计原则。它不定义具体命令名，而定义命令应该如何被设计、如何被审查。

## 1. Design Goal

Vibe Python CLI 的核心职责是作为 capability layer：
...
Python CLI 可以在单个命令内调用 `git`、`gh` 或其他底层工具，只要这些动作仍然服务于以下目标：
...
只要这些判断仍需依赖上层语义理解，责任就在 skill，而不在 Python CLI。
...
- [command-standard.md](/Users/jacobcy/src/vibe-center/wt-claude-refactor/docs/standards/v3/command-standard.md)
- [data-model-standard.md](/Users/jacobcy/src/vibe-center/wt-claude-refactor/docs/standards/v3/data-model-standard.md)
- [registry-json-standard.md](/Users/jacobcy/src/vibe-center/wt-claude-refactor/docs/standards/v3/registry-json-standard.md)
- [roadmap-json-standard.md](/Users/jacobcy/src/vibe-center/wt-claude-refactor/docs/standards/v3/roadmap-json-standard.md)

设计层只回答：

- Python CLI 是否误解了这些语义
- Python CLI 是否跨层混用了这些语义
- Python CLI 是否提供了足够原子能力让 skill 正确使用这些语义

## 3. Atomic Method Rule

每个 Python CLI 命令应尽量只表达一个清晰的能力边界。

这里不要求 Python CLI 退化成"只读一个字段"或"只写一个字段"。只要一组动作服务于同一个共享事实或同一个单一现场同步目标，就仍然可以视为一个能力。
...
skill 只能通过 Python CLI API：

- 查询共享状态
- 更新共享状态
- 创建、绑定和同步与共享真源相关的现场

如果 skill 为了完成工作不得不直写 JSON，说明 Python CLI 能力缺失。

这类问题应被归类为：

- `Capability Gap`

而不是让 skill 绕过 Python CLI。

## 6. Review Checklist

设计或审查一个 Python CLI 命令时，必须逐项回答：
...
判断 Python CLI 是否设计正确，不看它"聪不聪明"，只看两件事：
...
## 8. Application To Future Python CLI Work

后续所有 Python CLI 设计文件都必须显式回答：
...
如果一份 Python CLI 设计文件没有回答这些问题，则设计未完成。
