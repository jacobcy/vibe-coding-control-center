...
| 编排层 | `vibe-orchestra` | assignee issue pool 的事实观察、分诊、优先级调度、非 state label 治理 | broader repo issue pool 扫描、单 flow 执行、写代码、替 manager 决定实现步骤 |
...
| 审计旁路 | `vibe-check` | `task <-> flow` / worktree runtime / stale binding 审计与修复 | 规划排期、registry/roadmap-task 语义审计 |
