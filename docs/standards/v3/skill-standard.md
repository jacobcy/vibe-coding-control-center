...
本文档用于解决以下常见混淆：
- `skills/`、`.claude/skills/` (**Legacy**: `.agent/skills/`)、`npx skills`、`vibe3 skills` 的关系
...
| Skills 运行时链接 | 供 Agent 实际加载的 symlink 层 | `.claude/skills/` (**Legacy**: `.agent/skills/`) | `scripts/init.sh` / `vibe3 skills sync` | 运行时产物，非权威源 |
...
2. `.claude/skills/` (**Legacy**: `.agent/skills/`) 只做运行时链接，不直接手改业务逻辑。
...
1. 是否把 `skills/` 与 `.claude/skills/` (**Legacy**: `.agent/skills/`) 区分为"源码"与"运行时"？
