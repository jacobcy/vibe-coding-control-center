...
如果这些动作都服务于同一个共享真源同步目标，且不替 skill 决定任务拆分、issue/roadmap 归属、优先级和 next step，那么它仍可能是可接受的中等粒度 CLI 能力。
...
- 若只是围绕当前单一 flow 建立共享事实与现场落地，则可以保留为 CLI 能力
...
这意味着 skill 若想把某个 `repo issue` 拆成多个 task execution record 并建立统一关联，CLI 目前没有足够能力。
...
- CLI 越权替 skill 做任务拆分决策
- skill 误以为调用单个 CLI 命令就能完成业务逻辑
...
问题不在 workflow 文案，而在其依赖的 CLI 方法还不完整。
...
- 它也容易让 agent 误以为现有 CLI 已能直接完成 roadmap -> task 分配
...
- 标准原则以前写得不够显式，现在已补强
- CLI 可以调用 `git` / `gh` / worktree，但这些副作用必须服务于共享真源与单一现场同步
- `roadmap.current` 与 branch current 的语义边界现已明确
- `repo issue`、`roadmap item`、`task`、`flow` 的关系边界现已明确
- 大部分 skill 文案已经接受“CLI 是能力层，不是业务判断者”这一原则
- 当前主要问题不在 CLI 会不会调用 `git` / `gh`，而在它是否替 skill 做了语义判断，以及当前原子能力是否足够
