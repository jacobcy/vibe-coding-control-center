...
- 这些动作词只提供默认解释与执行提醒
- 若具体命令语义与本表冲突，以 [v3/command-standard.md](v3/command-standard.md) 为准
...
- 反歧义提醒：对 `vibe3 flow update` 而言，默认是在当前 worktree 中切入新的 flow / branch 现场，不等于新建物理 worktree
- 与 intake 关系：在 V3 流程中，`vibe3 task intake` 是将 issue 纳入本地管理的正式路径；`new` (或 `/vibe-new`) 通常作为规划与现场初始化的入口。
...
- 默认含义：启动或管理常驻基础设施服务（如 Orchestra server）
- 前置提醒：确认端口占用、服务健康度与日志路径
- 不要隐含什么：不要借 `serve` 执行具体的业务开发任务，它仅负责运行环境的维持

### 2.17 `scan`

- 默认含义：执行一次性的、只读的治理巡检或规则审计（不启动常驻服务）
- 前置提醒：确认扫描范围与规则集
- 不要隐含什么：不要在 `scan` 过程中自动执行修复或状态写入（除非是显式授权的 apply 动作）
