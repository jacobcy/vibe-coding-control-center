...
| 工具 | 类型 | 版本 | 安装方式 | 状态 |
|------|------|------|---------|------|
| claude-mem | plugin | 13.8.1 | `claude plugin install` | ✅ |
| rtk | CLI | 0.42.4 | brew/npm/cargo | ✅ |
| caveman | plugin | 0.1.0 | `claude plugin install` | ✅ |
| graphify | CLI + skill | 0.9.8 | `uv tool install graphifyy` | ✅ |
| spec-kit (specify) | CLI + skill | 0.12.4 | `uv tool install specify-cli` | ✅ |
| superpowers | plugin skill | 6.0.3 | `claude plugin install` | ✅ |
| codex | plugin | 1.0.2 | `claude plugin install` | ✅ |
| claude-hud | plugin | 0.3.0 | `claude plugin install` | ✅ |
| openspec/opsx | skills | - | `npx skills add` | ✅ |
| vibe skills | skills | - | `scripts/init.sh` | ✅ |
| exa search | MCP server | 3.3.9 | `mcpServers` config | ✅ |
| context7 | plugin | unknown | `claude plugin install` | ✅ |
...
```bash
# 由 init.sh 通过 skills.json 安装
npx skills add openspec/openspec --skill <skills>
```
...
在 `~/.claude/settings.json` 中配置（非 plugin 方式）：
