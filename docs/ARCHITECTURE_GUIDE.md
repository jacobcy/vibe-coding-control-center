...
| 层级 | 名称 | 职责 | 核心命令 |
|------|------|------|----------|
| **Tier 3** | **Cognitive / Governance** | 认知与治理层：负责全局策略、规则、Supervisor 治理。 | `serve`, `scan`, `check`, `mcp` |
| **Tier 2** | **Skill Layer** | 技能/编排层：负责 Flow 状态机、任务编排、Agent 执行。 | `flow`, `task`, `run`, `plan`, `review` |
| **Tier 1** | **Shell Layer** | 壳层/原子能力：提供原子级能力访问、状态读取与项目信息检索。 | `handoff`, `inspect`, `pr`, `ask` |
...
Tier 3 (Cognitive/Governance)
  L1  Orchestra / Heartbeat          -- 调度主循环 (Inspection Level)
  L2  Supervisor + Apply             -- 轻量治理执行 (Governance Execution Level)

Tier 2 (Skill Layer)
  L3  Manager / Plan / Run / Review  -- 核心编排流程 (Main Development Level)

Tier 1 (Shell Layer)
  L3/L4 Atomic Commands              -- 原子工具 (Atomic Collaboration Level)
```

**Worktree 语义**：
- **Tier 3 (L1)**: 无 worktree（`cwd=None`）
