...
| **[/vibe-task](.agent/workflows/vibe:task.md)** | 任务总览 | 处理跨 worktree 总览与 assignee issue 状态修复。 |
...
2. **V3 (Python)** — GitHub issue/flow/PR 管理主系统（`src/vibe3/`, `tests/vibe3/`）
...
2. **查看全局状态**: 运行 `vibe3 task status`（推荐，包含 assignee issue 总览）
