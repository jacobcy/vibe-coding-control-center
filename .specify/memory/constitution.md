...
=== Amendment 1.1.0 (2026-07-07) ===
Version change: 1.0.0 → 1.1.0
Rationale: 新增 Explore Before Spec workflow convention（#3327 vibe-explore extension）。
Type: MINOR（Spec-Kit Workflow Conventions 新增条目，非新 Core Principle / 非新 section）。
Modified:
  - Spec-Kit Workflow Conventions 新增 "Explore Before Spec" 条目
  - 扩展列表补 vibe-spec-bridge（artifact→handoff）+ vibe-explore（explore-before-spec）
Inference basis: #3327 设计决策 + 自动化指令（plan 不预注入上下文，agent 自用工具收集）
===
-->
...
- **扩展**：`superspec` v1.0.1（bridge）+ `vibe-spec-bridge`（artifact→handoff）+ `vibe-explore`（explore-before-spec），hooks 启用（auto_execute）。
- **Explore Before Spec**：非平凡 feature 的 specify 前，经 `vibe-explore` extension 的 mandatory `before_specify` hook 收集 graphify/claude-mem/ADR/已有 spec 上下文作为 **ephemeral evidence**；explore 非正式产物，不进 handoff（handoff 起点是 `@spec`）。对齐自动化指令：agent 自用工具收集上下文，不被 plan 预注入（见 #3327/#3333）。
- **双轨模型**：spec-kit（spec-driven 人机协作）与 vibe3 flow（issue-driven 自动化）是两条并行轨，经 `vibe-spec-bridge` 的 `after_*` hooks 单向桥接（spec-kit 产物 → vibe3 handoff refs），**不互相驱动**。orchestra 不全自动驱动 spec-kit 六阶段（#3332 rescope，对齐"全自动化效果不好"指令）。选用决策与 superspec 可用性见 [docs/standards/spec-kit-workflow-standard.md](../../docs/standards/spec-kit-workflow-standard.md)。
- **Git 追踪策略**：追踪 `memory/`、`specs/`、`templates/`、`workflows/`、`*.yml/json`；
  忽略 `.specify/extensions/`（安装产物）、`.specify/scripts/`（CLI 依赖）、
  `.specify/superpowers.yml`（可重生成的检测缓存）。详见项目 `.gitignore` 的
  「Spec-Kit」段落。
- **与 OpenSpec 的关系**：`openspec/config.yaml` 已追踪（骨架/种子），但 `specs/`、
  `changes/` 尚未实际使用。spec-kit 是本项目当前唯一的 spec-driven 工具链，避免双轨
  （原则 II）；openspec 留作可选对照，不与 spec-kit 并行激活。
...
**Version**: 1.1.0 | **Ratified**: 2026-07-03 | **Last Amended**: 2026-07-07
