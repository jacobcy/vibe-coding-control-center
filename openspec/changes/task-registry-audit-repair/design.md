...
vibe task audit
  ...
  └── Phase 3: Heuristic Audit (可选)
      └── PR commits/description → 任务识别
```

### D2: 实现位置（Shell vs Skill）

**Decision**: Shell 层提供原子操作，Skill 层编排核对流程

**Rationale**:
- 符合三层架构原则（Shell 物理真源，Skill 智能编排）
- Shell 层保持简洁，只提供基础的查询和修复命令
- Skill 层处理复杂的核对逻辑和用户交互

**Shell Commands** (新增):
- `vibe task audit --fix-branches` - 修复 null branch 字段
- `vibe task audit --check-branches` - 核对分支任务注册
...
### Phase 3: PR Task Detection (Optional)

**Scope**: 从 PR 识别完成的任务

**Success Criteria**:
- 识别出 PR 完成但未注册的任务
- 置信度分级，高置信度自动建议，低置信度需确认
