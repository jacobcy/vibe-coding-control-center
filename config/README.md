...
- **settings.yaml.template**: Project override template copied to `.vibe/settings.yaml` by `vibe init`
...
`config.yaml` and `settings.yaml` have different jobs:
- `.vibe/config.yaml` is project identity and convention metadata generated by
  `vibe init` (profile, features, labels, branch conventions, and resource paths).
- `.vibe/settings.yaml` is the project-level runtime override loaded into
  `VibeConfig` (orchestra, run, plan, review, and other execution settings).

Runtime settings are merged in this order, from lowest to highest priority:
1. Repo or installed default: `config/v3/settings.yaml`
2. Global user override: `~/.vibe/settings.yaml`
3. Target repo override: `.vibe/settings.yaml`

Legacy discovery still accepts older paths such as `config/settings.yaml` for
older checkouts, but new project overrides should use `.vibe/settings.yaml`.
