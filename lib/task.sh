...
_vibe_task_update() {
    ...
    # Check if task exists, if not, auto-create
    if ! jq -e --arg task_id "$task_id" '.tasks[]? | select(.task_id == $task_id)' "$registry_file" >/dev/null 2>&1; then
        log_warn "Task not found: $task_id. Auto-creating..."
        _vibe_task_add "$task_id" --id "$task_id" || return 1
    fi
    if [[ -n "$agent" ]]; then
...
_vibe_task_remove() {
    ...
    local indexed_branch; indexed_branch=$(jq -r --arg tid "$task_id" '.worktrees[]? | select(.current_task == $tid) | .branch // empty' "$worktrees_file" | head -1)
...
_vibe_task_sync() {
    ...
    if [[ -n "$tasks_list" ]]; then
        while read -r task_json; do
            ...
            jq --arg tid "$tid" --arg title "$title" --arg src "openspec/changes/$tid" '.tasks |= map(if .task_id == $tid then .title = $title | .framework = "openspec" | .source_path = $src else . end)' \
