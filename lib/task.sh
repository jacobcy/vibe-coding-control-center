...
_vibe_task_render() {
        ...
        jq -r --slurpfile registry "$registry_file" --slurpfile openspec "$openspec_tasks_file" '
            ...
            | ([.worktrees[]?]
                | if length == 0 then
                        ...
                        map(
                            . as $w
                            | ($all | map(select(.task_id == $w.current_task)) | .[0]) as $t
                            | [
                                    "- \($w.worktree_name // "-")",
                                    "  path: \($w.worktree_path // "-")",
                                    "  branch: \($w.branch // "-")",
                                    "  state: \($w.status // "-") \(if $w.dirty then "dirty" else "clean" end)",
                                    "  task: \($w.current_task // "-")",
                                    "  title: \($t.title // "-")",
                                    "  framework: \($t.framework // "vibe")",
                                    "  source: \($t.source_path // "-")",
                                    "  status: \($t.status // "-")",
                                    "  current subtask: \($t.current_subtask_id // "-")",
    ...
    echo "--- Task Registry Overview ---"
        jq -r --slurpfile openspec "$openspec_tasks_file" --arg show_all "$show_all" '
            (((.tasks // []) + ($openspec[0].tasks // []) | unique_by(.task_id)) as $all
            | ([$all[]
                    | select($show_all == "1" or ((.status // "") != "completed" and (.status // "") != "archived" and (.status // "") != "done" and (.status // "") != "skipped"))
                ]
                | if length == 0 then
