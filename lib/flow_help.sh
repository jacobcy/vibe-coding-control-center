...
Options for 'switch <name>':
  --branch <ref>     指定已有 flow 的分支来源提示（仅在 materialize 远端分支时使用）
  --save-stash       将当前未提交改动 stash 后带入已存在 flow
Parallel worktree:
...
Usage: vibe flow switch <name> [--branch <ref>] [--save-stash]
  --branch <ref>     在当前目录进入已存在 flow 时使用的分支来源提示
  --save-stash       将未提交改动 stash 后带入已存在 flow
