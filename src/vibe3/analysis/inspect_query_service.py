"""Inspect query service — GitHub-backed query logic for inspect commands.

Moved from commands/inspect_helpers.py to remove command-layer client
dependency. Commands should delegate to this module instead of constructing
clients directly.
"""
        ...
        if untracked_files:
            for file in changed_files:
                ...
                try:
                    changed_lines += max(
                        1,
                        len(Path(file).read_text(encoding="utf-8").splitlines()),
                    )
                except OSError:
