...
DEFAULT_PROMPTS_PATH = Path("assets/prompts/prompts.yaml")
...
def _resolve_prompts_path() -> Path:
    ...
    try:
        ...
        repo_path = repo_root / "assets" / "prompts" / "prompts.yaml"
        if repo_path.exists():
            return repo_path
...
def load_prompt_templates(prompts_path: Path | None = None) -> dict[str, Any]:
    """Load prompt templates from assets/prompts/prompts.yaml with defaults."""
