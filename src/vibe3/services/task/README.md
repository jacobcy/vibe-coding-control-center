...
| `status.py` | 454 | Task 状态仪表盘数据获取（perform_status_fetch / build_api task data） |
...
- `perform_status_fetch` — 获取 task 状态原始 snapshot/flows/issues（供 CLI 仪表盘直接消费）
...
1. **与 flow 模块**:
   ...
   - Task 状态查询聚合 Flow 状态 (perform_status_fetch 包含 flow 信息)
