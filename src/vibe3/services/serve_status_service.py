...
class ServeStatusService:
    ...
    def _display_active_jobs(self) -> None:
        """Display active and recent jobs from the actor registry."""
        from vibe3.services.job_monitor_service import JobMonitorService

        service = JobMonitorService()
        snapshot = service.snapshot()

        self.console.print("[bold]Active Jobs:[/bold]")

        if not snapshot.active_jobs and not snapshot.recent_jobs:
            self.console.print("  No active jobs")
            self.console.print()
            return
