...
class GitClient:
    ...
    def get_diff_hunk_ranges(
        ...
        if source.type == ChangeSourceType.UNCOMMITTED:
            untracked_files = set(self.get_untracked_files())
        ...
        try:
            ...
            diff = self._get_file_diff(file_path, source)
    ...
    def _get_file_diff(self, file_path: str, source: ChangeSource) -> str:
        ...
        if source.type == ChangeSourceType.COMMIT:
            ...
            return self._run(["diff", f"{source.sha}^", source.sha, "--", file_path])
            ...
            if source.pr_number not in self._pr_diff_cache:
                self._pr_diff_cache[source.pr_number] = self._github_client.get_pr_diff(
                    source.pr_number
                )
            full_diff = self._pr_diff_cache[source.pr_number]
            return self._extract_file_diff(full_diff, file_path)
        else:
            # Uncommitted changes
            return self._run(["diff", "HEAD", "--", file_path])
